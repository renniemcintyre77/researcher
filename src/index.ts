    ...
    if(key && key !== ""){
        ...
        if (existingQuestion && existingQuestion.content !== null) {
            ...
            if(existingQuestion && existingQuestion.searchQuery && existingQuestion.searchQuery !== ""){
                ...
                const perplexity = await getSearchSop(existingQuestion.key);
                const actionComplete = async (eventData: any) => {

                    for(let i=0; i<eventData.length; i++){

                        console.log(`Received: ${eventData[i].key}`);

                        if (eventData[i].key === 'validator') {
                            res.write(`data: ${JSON.stringify({ type: 'validator', data: eventData[i].data })}\n\n`);;
                        }

                        if (eventData[i].key === 'images') {
                            const images = eventData[i].data.images.map((image: any) => { return { thumbnail: image.thumbnailUrl, image: image.imageUrl }}).slice(0, 8);
                            res.write(`data: ${JSON.stringify({ type: 'images', data: images })}\n\n`);
                        }

                        if (eventData[i].key === 'relatedQuestions') {
                            res.write(`data: ${JSON.stringify({ type: 'relatedQuestions', data: eventData[i].data })}\n\n`);
                        }

                        if (eventData[i].key === 'summary') {
                            res.write(`data: ${JSON.stringify({ type: 'summary', data: eventData[i].data })}\n\n`);
                        }

                    }
                }

                perplexity.onActionComplete(actionComplete);
                const response = await perplexity.invoke({ searchQuery: existingQuestion.searchQuery });
                ...
                perplexity.removeActionCompleteListener(actionComplete);
                res.write(`data: ${JSON.stringify({ type: 'done' })}\n\n`);
