...
// src/tools/serper.ts
var SerperTool = class {
  name = "serper";
  description = "serper";
  /**
   * Uses Serper API to get images related to the search query.
   *
   * @param {string} searchQuery - The query to use for the search.
   * @returns {Promise<ToolResponse>} A promise that resolves to the tool response.
   */
  invoke = async (searchQuery) => {
    if (!process.env.SERPER_API_KEY) {
      throw Error("SERPER API KEY Not Found!");
    }
    ...
    try {
      ...
      redisClient.set(checksum, JSON.stringify(response.data), "EX", Number(process.env.CACHE_TTL));
...
var serperTool = new SerperTool();
var serper_default = serperTool;

// src/tools/serperimages.ts
import axios2 from "axios";
import crypto2 from "crypto";
var SerperImagesTool = class {
  name = "serper";
  description = "serper";
  /**
   * Uses Serper API to get images related to the search query.
  ...
  invoke = async (searchQuery) => {
    ...
    const checksum = crypto2.createHash("sha256").update(JSON.stringify(config)).digest("hex");
    const result = await redisClient.get(checksum);
    if (result) {
      return JSON.parse(result);
    }
...
var serperImagesTool = new SerperImagesTool();
var serperimages_default = serperImagesTool;
...
// src/tools/searchsummary.ts
var SearchSummaryTool = class {
  name = "SearchSummaryTool";
  description = "Summarize a search result";
  /**
  * Creates a summary of the search results based on the user query.
  * Cites the search results at the end of each sentence if applicable.
  *
  * @param {Agent} agent - The agent to use for the summary.
  * @param {llm} model - The LLM to use for the summary.
  * @param {string} searchQuery - The query to use for the search.
  * @param {string} research - The search results to summarize.
  * @returns {Promise<ToolResponse>} A promise that resolves to the tool response.
  */
  invoke = async (agent, model, research, searchQuery) => {
    const prompt = ChatPromptTemplate.fromMessages([
      agent.getSystemPrompt(),
      HumanMessagePromptTemplate.fromTemplate(`
            You are given a user question, and please write clean, concise and accurate answer to the question.
            You will be given a set of related search results to the question, each starting with a reference number like [[citation:x]],
            where x is a number. Please use the context and cite the search results at the end of each sentence if applicable.

            Your answer must be correct, accurate and written by an expert using an unbiased and professional tone.
            Do not give any information that is not related to the question, and do not repeat.
            Say "information is missing on" followed by the related topic, if the given search results do not provide sufficient information.

            Please cite the search results with the reference numbers, in the format [citation:x].
            If a sentence comes from multiple search results, please list all applicable citations, like [citation:3][citation:5].
            ...
            Remember, don't blindly repeat the search results verbatim. And here is the user question: {{query}}

            {{format_instructions}}
            `, { templateFormat: "mustache" })
    ]);
    let researchBody = research.map((c, i) => `[[citation:${i + 1}]] ${c.snippet}`).join("\n\n");
    ...
    const citationNumbers = Array.from(new Set(Array.from(response.matchAll(/\[citation:(\d+)\]/g), (m) => parseInt(m[1])))).sort((a, b) => a - b);
    const citedResearch = citationNumbers.map((index) => {
      return {
...
var searchSummaryTool = new SearchSummaryTool();
var searchsummary_default = searchSummaryTool;
export {
  searchsummary_default as SearchSummaryTool,
  serperimages_default as SerperImagesTool,
  serper_default as SerperTool
};
