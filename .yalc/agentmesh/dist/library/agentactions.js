...
var SearchSummaryAction = class extends agentaction_default {
  async runAction(data) {
    ...
    let researchBody = research.slice(0, 15).map((c, i) => `[[citation:${i + 1}]] ${c.body}`).join("\n\n");
...
var SearchSummaryAction_default = SearchSummaryAction;
  ...
  async runAction(data) {
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      ...
      HumanMessagePromptTemplate.fromTemplate(`
            ...
            Each related question should be no longer than 20 words.
    ...
    const parser = StructuredOutputParser.fromZodSchema(
      z.array(
        z.string().describe("follow up question")
      )
    );
    const response = await prompt.pipe(this.llm).pipe(parser).invoke({
      role: this.agent.role,
